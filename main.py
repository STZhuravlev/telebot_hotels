...
def get_loading_photo(message):
    ...
    if loading_photo == 'Да' or loading_photo == 'Нет':
        ...
        if loading_photo == 'Да':
            ...
            bot.register_next_step_handler(message, get_count_photos)
        else:
            set_date_in(message)
    else:
...
def cal(callback_query):
    ...
    result, key, step = DetailedTelegramCalendar(calendar_id=1, locale='ru', min_date=date_min, max_date=date_max).process(callback_query.data)
...
def get_date_input(chat_id):
    ''' def get_date_input - поллучаем дату заезда'''
...
def date_out(chat_id):
    ...
    calendar, step = DetailedTelegramCalendar(calendar_id=2, locale='ru', min_date=date_today_min,max_date=date_today_max).build()
...
def cal(callback_query):
    ...
    передает ответ в БД, и переходит к следующему шагу '''
    date_today_min = (datetime.strptime(get_date_input(chat_id=callback_query.message.chat.id), "%Y-%m-%d") + timedelta(days=1)).date()
    date_today_max = date_today_min + timedelta(days=90)
    result, key, step = DetailedTelegramCalendar(calendar_id=2, locale='ru',
                                                 min_date=date_today_min,max_date=date_today_max).process(callback_query.data)
    if not result and key:
        bot.edit_message_text(f"Выберите {LSTEP[step]}",
                              callback_query.message.chat.id,
                              callback_query.message.message_id,
